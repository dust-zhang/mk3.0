...
-(void) initController
{
    self._viewWhiteBg = [[UIView alloc ] initWithFrame:CGRectMake(posx, 10, SCREEN_WIDTH-30, 340/2)];
    ...
    self._labelGoodsName = [[UILabel alloc ] initWithFrame:CGRectMake(posx, self._labelCutLine1.frame.origin.y+self._labelCutLine1.frame.size.height+15, SCREEN_WIDTH-60, 15)];
    ...
    self._labelOrderNo = [[UILabel alloc ] initWithFrame:CGRectMake(posx, self._labelCutLine2.frame.origin.y+self._labelCutLine2.frame.size.height+15, SCREEN_WIDTH, 60)];
...
-(void) setOrderCellText:(MyOrderListModel*)model index:(NSInteger)cellIndex time:(NSNumber *)currentTime
{
    ...
    CGSize size = [Tool CalcString:str fontSize:MKBOLDFONT(14) andWidth:SCREEN_WIDTH-60];
    self._viewWhiteBg.frame = CGRectMake(posx, 10, SCREEN_WIDTH-30, 320/2+ size.height+([model.goodsOrderList count] -1)*15);
    ...
    self._labelGoodsName.frame =CGRectMake(posx, self._labelCutLine1.frame.origin.y+self._labelCutLine1.frame.size.height+15, SCREEN_WIDTH-60, size.height);
...
{
    if ([self._deleteGoodsDelegate respondsToSelector:@selector(onButtonDeleteOrder:)])
    {
        [self._deleteGoodsDelegate onButtonDeleteOrder:btn ];
    }
}

-(void) refreshUnPayOrder
{
    if ([self._deleteGoodsDelegate respondsToSelector:@selector(refreshUnPayOrder)])
    {
        [self._deleteGoodsDelegate refreshUnPayOrder ];
    }
}
