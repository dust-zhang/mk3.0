...
-(id)initWithFrame:(CGRect)frame model:(MovieModel*)mModel
{
    ...
    if (self)
    {
        ...
        _labelScan.textColor = RGBA(117, 112, 255, 1);
...
-(void)refreshFrame
{
    ...
    if (self.movieModel.versionList.count>0)
    {
        ...
        if (self.movieModel.versionList.count>0)
        {
            ...
            CGFloat leftVWidth = SCREEN_WIDTH;
            for (int k = 0 ; k<self.movieModel.versionList.count; k++)
            {
                ...
                UILabel* labelFeature = [[UILabel alloc]initWithFrame:CGRectMake(originX, versionY, featureWidth,15)];
...
>>>>>>> 19d68b13d1ef73ae01085dbe4eca57157402ad05
    ...
    if (arrStill.count>0)
    {
        ...
        if (arrStill.count>=3)
        {
            ...
            _viewScan.frame = CGRectMake(0, scrollPhoto.frame.origin.y+scrollPhoto.frame.size.height+12, SCREEN_WIDTH, 20);
            _labelScan.frame = CGRectMake((SCREEN_WIDTH-scanW)/2, 0, scanW, 20);
            _imageScan.frame = CGRectMake(_labelScan.frame.origin.x+scanW+4, 11/2, 9/2, 18/2);
    ...
    if (arrTip.count>0)
    {
        ...
        for (int i = 0; i<arrTip.count; i++)
        {
            ...
            UILabel* labelTip = [[UILabel alloc]initWithFrame:CGRectZero];
            labelTip.text = arrTip[i];
            labelTip.font = MKFONT(10);
            labelTip.lineBreakMode = NSLineBreakByTruncatingTail;
            labelTip.numberOfLines = 1;
            [labelTip sizeToFit];
            labelTip.textColor = RGBA(106, 82, 25, 1);
            labelTip.backgroundColor = [UIColor clearColor];
            
            CGFloat labelTipW = labelTip.frame.size.width;
            if (labelTipW > SCREEN_WIDTH-40-18)
            {
                labelTipW = SCREEN_WIDTH-40-18;
            }
            labelTip.frame = CGRectMake(9, 1, labelTipW, 33/2);
            
            UIImageView* tipLeft = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, 9, 35/2)];
            tipLeft.image = [UIImage imageNamed:@"poster_tip_left.png"];
            tipLeft.backgroundColor = [UIColor clearColor];
            [tipView addSubview:tipLeft];
            
            UIImageView* tipMiddle = [[UIImageView alloc]initWithFrame:CGRectMake(tipLeft.frame.size.width, 1, labelTipW, 33/2)];
            tipMiddle.image = [UIImage imageNamed:@"poster_tip_middle.png"];
            tipMiddle.backgroundColor = [UIColor clearColor];
            [tipView addSubview:tipMiddle];
            
            UIImageView* tipRight = [[UIImageView alloc]initWithFrame:CGRectMake(tipMiddle.frame.origin.x+tipMiddle.frame.size.width, 1, 9, 33/2)];
            tipRight.image = [UIImage imageNamed:@"poster_tip_right.png"];
            tipRight.backgroundColor = [UIColor clearColor];
            [tipView addSubview:tipRight];
            
            tipView.frame = CGRectMake(20, _labelTip.frame.origin.y+_labelTip.frame.size.height+15+i*tipH+i*8, tipRight.frame.origin.x+tipRight.frame.size.width, tipH);
            
            [tipView addSubview:labelTip];
//            UILabel* labelTip = [[UILabel alloc]initWithFrame:CGRectMake(20, _labelTip.frame.origin.y+_labelTip.frame.size.height+15+i*tipH+i*8, SCREEN_WIDTH - 20, tipH)];
//            labelTip.text = arrTip[i];
//            labelTip.font = MKFONT(10);
//            labelTip.textColor = RGBA(106, 82, 25, 1);
//            labelTip.backgroundColor = [UIColor orangeColor];
//            [self addSubview:labelTip];
        }
        backViewH = 64 + _labelTip.frame.origin.y+_labelTip.frame.size.height+15+arrTip.count*tipH+(arrTip.count-1)*8;
    }
