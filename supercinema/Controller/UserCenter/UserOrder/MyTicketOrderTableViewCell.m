...
@implementation MyTicketOrderTableViewCell
...
-(void) initController
{
    UIView *viewWhiteBg = [[UIView alloc ] initWithFrame:CGRectMake(posx, 10, SCREEN_WIDTH-30, (88+128+ 30+30+20+24+30+24+30-14)/2)];
    ...
    self._labelFilmName = [[UILabel alloc ] initWithFrame:CGRectMake(posx, self._labelCutLine1.frame.origin.y+self._labelCutLine1.frame.size.height+15, SCREEN_WIDTH-60, 15)];
    [self._labelFilmName setFont:MKBOLDFONT(15)];
    [self._labelFilmName setBackgroundColor:[UIColor clearColor]];
    [self._labelFilmName setTextColor:RGBA(51, 51, 51, 1)];
    [viewWhiteBg addSubview:self._labelFilmName];
    
    self._labelFilmDetail = [[UILabel alloc ] initWithFrame:CGRectMake(posx, self._labelFilmName.frame.origin.y+self._labelFilmName.frame.size.height+10, SCREEN_WIDTH-60, 12)];
    [self._labelFilmDetail setFont:MKFONT(12)];
    [self._labelFilmDetail setBackgroundColor:[UIColor clearColor]];
    [self._labelFilmDetail setTextColor:RGBA(51, 51, 51, 1)];
    [viewWhiteBg addSubview:self._labelFilmDetail];
    
    self._labelSum = [[UILabel alloc ] initWithFrame:CGRectMake(posx, self._labelFilmDetail.frame.origin.y+self._labelFilmDetail.frame.size.height+15, SCREEN_WIDTH-60, 12)];
...
-(void) setOrderCellText:(MyOrderListModel*)model index:(NSInteger)cellIndex time:(NSNumber *)currentTime
{
    ...
    [self._labelFilmName  setText:model.ticketOrder.movieTitle];
    [self._labelFilmDetail setText:[NSString stringWithFormat:@"%@ 电影票(%@张)",
    ...
    self._labelOrderNo.frame = CGRectMake(posx, self._labelCutLine2.frame.origin.y+self._labelCutLine2.frame.size.height, SCREEN_WIDTH-60, 62.6);
...
{
    if ([self._deleteTicketDelegate respondsToSelector:@selector(onButtonDeleteOrder:)])
    {
        [self._deleteTicketDelegate onButtonDeleteOrder:btn ];
    }
}

-(void) refreshUnPayOrder
{
    if ([self._deleteTicketDelegate respondsToSelector:@selector(refreshUnPayOrder)])
    {
        [self._deleteTicketDelegate refreshUnPayOrder ];
    }
}
