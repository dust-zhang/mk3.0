...
-(void)reloadMemberInfo
{
    ...
    [ServicesRedPacket getRedPacketList:[NSNumber numberWithInt:0] cinemaId:[NSNumber numberWithInt:[[Config getCinemaId] intValue]] cardItemId:self._memberCardDetialModel.cinemaCardItemId goodsIdAndCountList:self._arrayGoods ticketCount:[NSNumber numberWithInt:0] array:^(NSMutableArray *array) {
        ...
        for (int i = 0; i<array.count; i++)
        {
            ...
            RedPacketCellVO *cellModel = [self createCellVO:tmpRedPacket];
            //将初始化数据放入VO缓存数据中
            [arrCacheData addObject:cellModel];
            
            RedPacketCellVO *cellModel1 = [self createCellVO:tmpRedPacket];
...
{
    //顶部View
    [self._labelTop setText:self._memberModel.cinema.cinemaName];
...
/****************确认订单****************/
    ...
    [_labelActualMoney setText:[NSString stringWithFormat:@"%@",[Tool PreserveTowDecimals:self._memberCardDetialModel.price]]];
    ...
    [_labelLookDetailInCouponPrice setText:[NSString stringWithFormat:@"￥-%ld",priceDetail.dicountPrice]];
...
>>>>>>> adee57a5ee1e4f62ddf54f961613121fb13efe3f
...
-(void)onBtnRightDetails
{
    ...
    //支付按钮
    _btnRealPricePay = [[UIButton alloc] initWithFrame:CGRectMake(15, _tabRight.frame.origin.y+_tabRight.frame.size.height+10, SCREEN_WIDTH-15*2, 40)];
    ...
    [_btnRealPricePay addTarget:self action:@selector(onBtnRealPricePay) forControlEvents:UIControlEventTouchUpInside];
...
-(void)refreshDikouPrice:(RedPacketPriceVO *)countPrice
{
    ...
    else
    {
        ...
        {
            tempDiscountPrice = countPrice.sumPrice;
        }
        else
        {
    ...
    //确认订单 - 优惠券
    _labelLookDetailInCouponPrice.text = [NSString stringWithFormat:@"¥-%@",[Tool PreserveTowDecimals:[NSNumber numberWithFloat:tempDiscountPrice]]];
    
    //权益详情 - 实付金额
    _btnRealPricePay.titleLabel.text = [NSString stringWithFormat:@"实付金额：¥%@",[Tool PreserveTowDecimals:[NSNumber numberWithFloat:realPrice]]];
