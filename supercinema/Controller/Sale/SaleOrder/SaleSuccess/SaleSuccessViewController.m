...
@interface SaleSuccessViewController ()

@end

@implementation SaleSuccessViewController
...
-(void)initCtrl
{
    ...
    //TabView
    _tabViewSaleSuccess = [[UITableView alloc] initWithFrame:CGRectMake(0, _viewTopBG.frame.origin.y+_viewTopBG.frame.size.height, SCREEN_WIDTH, SCREEN_HEIGHT-(_viewTopBG.frame.origin.y+_viewTopBG.frame.size.height))];
    _tabViewSaleSuccess.delegate = self;
    _tabViewSaleSuccess.dataSource = self;
    [_tabViewSaleSuccess setBackgroundColor:RGBA(246, 246, 251, 1)];
    [_tabViewSaleSuccess setSeparatorColor:[UIColor clearColor]];
    [self.view addSubview:_tabViewSaleSuccess];
...
{
    __weak SaleSuccessViewController *weakself = self;
    [FVCustomAlertView showDefaultLoadingAlertOnView:self.view withTitle:@"正在出小卖，请稍候..." withBlur:NO allowTap:NO];
    [ServicesOrder getOrderDetail:self._orderId model:^(OrderInfoModel *model)//@"810000196188"   self._orderId
     {
         weakself._modelOrderInfo = model;
         
         [weakself reloadFrameAndData];
         [_tabViewSaleSuccess reloadData];
...
{
    self._strGoodsOrderId = goodsOrderId;
...
-(void)directlyReceiveGoods
{
    ...
    [ServicesGoods exchangeGoods:self._strGoodsOrderId code:nil model:^(RequestResult *model)
     {
         [FVCustomAlertView hideAlertFromView:weakself.view fading:YES];
         GoodsListCardPackModel *goodsListCardPackModel =self._modelOrderInfo.goodsOrderList[_exchangeIndex];
         goodsListCardPackModel.useStatus = @1;
         [_tabViewSaleSuccess reloadData];
         
     } failure:^(NSError *error) {
         [FVCustomAlertView hideAlertFromView:weakself.view fading:YES];
         GoodsListCardPackModel *goodsListCardPackModel =self._modelOrderInfo.goodsOrderList[_exchangeIndex];
         goodsListCardPackModel.useStatus = @0;
...
-(void)textChange:(UITextField *)textField
{
    if ([_textField.text length] >= 4)
    {
        ...
        [_textField resignFirstResponder];
        [FVCustomAlertView showDefaultLoadingAlertOnView:self.view withTitle:@"加载中..." withBlur:NO allowTap:NO];
        [ServicesGoods exchangeGoods:self._strGoodsOrderId code:_textField.text model:^(RequestResult *model)
         {
             [FVCustomAlertView hideAlertFromView:weakself.view fading:YES];
...
{
    UITableViewCell *cell = (UITableViewCell *)[self tableView:_tabViewSaleSuccess cellForRowAtIndexPath:indexPath];
...
{
    return [self._modelOrderInfo.goodsOrderList count]+1;
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
   ...
   if (indexPath.row < [self._modelOrderInfo.goodsOrderList count])
    {
        //小卖信息
        ...
        [tableCell setSmallSaleDescribeCellFrameData:[self._modelOrderInfo.goodsOrderList objectAtIndex:indexPath.row] orderWhileModel:_orderModel];
