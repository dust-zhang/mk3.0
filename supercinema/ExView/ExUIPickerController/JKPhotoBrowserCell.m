...
- (void)setAsset:(ALAsset *)asset
{
    if (_asset != asset)
    {
        ...
        else
        {
            ...
            _asset = asset;
            NSString * str = [NSString stringWithFormat:@"%@",_asset];
            
            NSString  *name = @"loading_black_280x180.gif";
            NSString  *filePath = [[NSBundle bundleWithPath:[[NSBundle mainBundle] bundlePath]] pathForResource:name ofType:nil];
            NSData  *imageData = [NSData dataWithContentsOfFile:filePath];
//            [self.imageView setContentMode:UIViewContentModeCenter];
            [self.imageView sd_setImageWithURL:[NSURL URLWithString:str] placeholderImage:[UIImage sd_animatedGIFWithData:imageData] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
                if (error) {
                    [self.imageView  setImage:[UIImage imageNamed:@"ImageNull_750x418.png"]];
                }
