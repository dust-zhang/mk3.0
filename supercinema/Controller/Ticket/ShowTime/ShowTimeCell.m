...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    _imgActivity = [[UIImageView alloc]initWithFrame:CGRectZero];
    _imgActivity.backgroundColor = [UIColor clearColor];
    [self.contentView addSubview:_imgActivity];
    
    _imgActivityLeft = [[UIImageView alloc]initWithFrame:CGRectZero];
    _imgActivityLeft.image = [UIImage imageNamed:@"img_activity_left.png"];
    [_imgActivity addSubview:_imgActivityLeft];
    
    _imgActivityCenter = [[UIImageView alloc]initWithFrame:CGRectZero];
    _imgActivityCenter.image = [UIImage imageNamed:@"img_activity_center.png"];
    [_imgActivity addSubview:_imgActivityCenter];
    
    _labelActivity = [[UILabel alloc]initWithFrame:CGRectZero];
    _labelActivity.textColor = [UIColor whiteColor];
    _labelActivity.font = MKFONT(9);
    _labelActivity.textAlignment = NSTextAlignmentCenter;
    [_imgActivityCenter addSubview:_labelActivity];
    
    _imgActivityRight = [[UIImageView alloc]initWithFrame:CGRectZero];
    _imgActivityRight.image = [UIImage imageNamed:@"img_activity_right.png"];
    [_imgActivity addSubview:_imgActivityRight];
...
-(void)setData:(ShowTimesModel*)showTModel
{
    ...
    _labelActivity.text = showTModel.activity.activityTitle;
...
-(void)setMoviePrice:(ShowTimesModel*)showTModel
{
    ...
    {
        _labelFirst.text = [NSString stringWithFormat:@"%@：",firstPriceModel.cinemaCardName];
    }
...
-(void)layoutFrame:(int)isLastCell
{
    CGFloat space = 0;
    CGFloat originX = 150;
    if (_labelShowTimeType.text.length>0)
    {
        ...
        if (isLastCell == 0 || isLastCell == 2)
        {
            //第一个或唯一cell
            _imageShowTimeType.frame = CGRectMake(15, 30, 12, 12);
            _labelShowTimeType.frame = CGRectMake(15+12+10, 30, 100, 12);
        }
    }
    else
    {
    ...
    _imageTimeLine.frame = CGRectMake(20, _imageShowTimeType.frame.origin.y+_imageShowTimeType.frame.size.height+space, 2, 90+space);
    ...
    if (_labelActivity.text.length>0)
    {
        _imgActivity.frame = CGRectMake(15, _labelStartTime.frame.origin.y-18, SCREEN_WIDTH-15, 40/2);
        _imgActivityLeft.frame = CGRectMake(0, 0, 6, 20);
        CGFloat widthAct = [Tool calStrWidth:_labelActivity.text height:9];
        _imgActivityCenter.frame = CGRectMake(6, 0, widthAct+6, 16);
        _labelActivity.frame = CGRectMake(0, 0, widthAct, 16);
        _imgActivityRight.frame = CGRectMake(_imgActivityCenter.frame.origin.x+_imgActivityCenter.frame.size.width, 0, 7, 16);
    }
    
    if (isLastCell == 1 || isLastCell == 2)
    {
        //最后一个cell
